...
setTimeout(function () {
    ...
    var query =`select distinct em.Id,em.CEEmpresa from Lappiz_Users u
    join SafeWork_Lappiz_EmpresaContratista em on em.Id = u.EmpresaContratistaFK
    ...
    $('#Filtrar')[0].onclick=function(){
        debugger;
        if($('#Tipo').val()!=null && $('#Estado').val() !=null && $('#fechainicial').val() !='' && $('#fechafinal').val()!='' && $('#Empresa').val()!=null && $('#sedeEmpresa').val()!=null && $('#areaSede').val()!=null && $('#Empresa').val()!='' && $('#sedeEmpresa').val() !='' && $('#areaSede').val()!=''){
            var Tipos = $('#Tipo').val()
            var txtTipos ='';
