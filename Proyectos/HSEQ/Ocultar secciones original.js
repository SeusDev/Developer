...
if (location.href.includes('497517a2-d20f-47a3-a3a2-9705d7d9c2bf')) {
    ...
    setTimeout(function () {
        ...
        if (e.isNew) {
            ...
            var textTabs = ["Auditoria", "Equipos de medición", "Autorización", "Incidentes", "Cierre Final","Excavación"];
            ocultarMostrarTabs(textTabs, false);
            toastr.info("Recuerde que para poder proceder a firmar, debe cumplir con todo lo requerido en las secciones de Equipos y Precauciones")
            ...
            $('#E21').kendoMultiSelect({
                dataTextField: 'nombre',
                dataValueField: 'nombre',
                dataSource: [{ nombre: 'Arnés para rescates en incendios' }, { nombre: 'Arnés para rescates en espacios reducido' }, { nombre: 'Cesta' }, { nombre: 'Tripode' }],
                value: [{ nombre: 'Arnés para rescates en incendios' }, { nombre: 'Arnés para rescates en espacios reducido' }, { nombre: 'Cesta' }, { nombre: 'Tripode' }]
            })
            $('#E20').kendoMultiSelect({
                dataTextField: 'nombre',
                dataValueField: 'nombre',
                dataSource: [{ nombre: 'Malla puesta a tierra' }, { nombre: 'Electrodo de Tierra' }, { nombre: 'Varilla de cobre' }],
                value: [{ nombre: 'Malla puesta a tierra' }, { nombre: 'Electrodo de Tierra' }, { nombre: 'Varilla de cobre' }]
            })

            // deshabilitar Hora fin en el tab general cuando se seleccione la hora
            $('label:contains(Hora fin)')[0].nextElementSibling.children[0].children[0].disabled = true;

        }
        clearInterval(sessionStorage.setIntervalNotificacion)
        var dataMultiSelect = [{ Tipo: "Trabajo en Caliente" }, { Tipo: "Trabajo en Frío" }, { Tipo: "Trabajo en espacio confinado" }, { Tipo: "Trabajo en alturas" }, { Tipo: "Trabajo eléctrico" }, { Tipo: "Trabajo de excavación" }];
        ...
        $("#TipoPermiso").kendoMultiSelect({
            ...
            change: function (e) {
                setTimeout(() => {
                    ...
                    ocultarMostrarTabs(["Equipos de medición","Excavación"], false)
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        switch (tipoTrabajo) {
                            ...
                            case "Trabajo en espacio confinado":
                                ...
                                break
                                case "Trabajo de excavación":
                                ocultarMostrarTabs(["Excavació"], true)
                                break
        ...
        if (!e.isNew) {
            ...
            if (e.dataItem.E21 != null && e.dataItem.E21 != '' && e.dataItem.E21 != undefined) {
                var dataE21 = e.dataItem.E21.split(',');
                var dataE21Aux = [];
                for (var i = 0; i < dataE21.length; i++) {
                    dataE21Aux.push({ nombre: dataE21[i] });
                }
                $('#E21').kendoMultiSelect({
                    dataTextField: 'nombre',
                    dataValueField: 'nombre',
            ...
            if (e.dataItem.E20 != null && e.dataItem.E20 != '' && e.dataItem.E20 != undefined) {
                var dataE20 = e.dataItem.E20.split(',');
                var dataE20Aux = [];
                for (var i = 0; i < dataE20.length; i++) {
                    dataE20Aux.push({ nombre: dataE20[i] });
                }
                $('#E20').kendoMultiSelect({
                    dataTextField: 'nombre',
                    dataValueField: 'nombre',
        ...
        if (!e.isNew) {
            ...
            // ocutar boton(+) detalles
            $('label:contains(Riesgo por permiso / Permiso de trabajo)')[0].parentElement.children[1].children[0].children[0].children[0].children[0].children[0].style.display = 'none'
            $('label:contains(Personal)')[0].parentElement.children[1].children[0].children[0].children[0].children[0].children[0].style.display = 'none'
            $('label:contains(Herramientas por permiso / Permiso)')[0].parentElement.children[1].children[0].children[0].children[0].children[0].children[0].style.display = 'none'
            $('label:contains(Medicion / Permiso de trabajo)')[0].parentElement.children[1].children[0].children[0].children[0].children[0].children[0].style.display = 'none'
        ...
        if (rolId == ejecutante) {
            ...
            // deshabilitar check cierre autoridad atea
            document.querySelectorAll('#SectionsFields > div > div > label > input')[12].disabled=true
            ...
            if (e.dataItem.FirmaEjecutante == undefined || e.dataItem.FirmaEjecutante == null || e.dataItem.FirmaEjecutante == '') {
                ...
                if (e.isNew) {
                    ...
                    if (e.dataItem.EstadoPermisoTrabajo != 'Aprobado' && e.dataItem.EstadoPermisoTrabajo != 'En proceso de cierre' && e.dataItem.EstadoPermisoTrabajo != 'Cerrado') {
                        ...
                        ocultarMostrarTabs(["Auditoria"], true);
                        BloquearCampos();
                        $("#2_Autorización_del_supervisor").hide();
                        $("#3_Autoridad_de_area").hide();
                        $("#Profesional").hide();
